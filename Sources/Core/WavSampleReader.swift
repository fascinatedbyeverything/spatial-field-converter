...
public final class WavSampleReader {
    ...
    public func readNextBlock(maxFrames: Int) throws -> PCMBlock? {
        ...
        // 32-bit IEEE float passthrough — samples are already in [-1, +1], no conversion needed.
        // Apple Silicon is little-endian native, so bindMemory(to: Float.self) reads correctly.
        if metadata.sampleFormat == .pcmFloat && metadata.bitsPerSample == 32 {
